...
| **📱 EPUB / Kindle** *(transversal)* | **Não gera conteúdo — converte formato.** Transforma qualquer artefato acima num EPUB 3 para o Kindle Paperwhite, com o furigana preservado. **Automático só para a aula** (Regra 13(b2)); para as demais modalidades é **sob demanda**. | [EPUB.md](./Modalidades/EPUB.md) | `"EPUB Aula X"` / `"Kindle Aula X"` · qualificado por modalidade, com `EPUB` ou `Kindle` como prefixo: `"EPUB Reading Aula X"` / `"Kindle Reading Aula X"`, `"EPUB Teste Aula X"` / `"Kindle Teste Aula X"`, `"EPUB Lacunas Aula X"` / `"Kindle Lacunas Aula X"`, `"EPUB Ditado Aula X"` / `"Kindle Ditado Aula X"` |
...
9. Para a modalidade **EPUB / Kindle**: converte um artefato **já existente** com `node scripts/build_epub.js <arquivo> --upload`. É transversal e **sob demanda** — a única exceção é a aula, cujo EPUB já sai automático pela Regra 13(b2) de `JLPTN5.md`. Não lê nem escreve `Progress.md`: não avalia nada. Se a fonte não estiver mais em disco, baixe-a do Drive antes de converter.
