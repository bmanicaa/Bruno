...
9. **OUTPUT & DRIVE EXPORT**: Segue o fluxo completo descrito na Regra 13 de `JLPTN5.md` e em `Filters/HTML/HTML_Lesson.md` §4.4. Resumo: gerar HTML5, salvar temporariamente, upload via `upload_to_gdrive.js`, gerar a versão e-reader do MESMO HTML via `scripts/build_epub.js --upload` (§4.7 — EPUB 3 com furigana, para o Kindle Paperwhite), apagar os arquivos locais, confirmar no chat.
