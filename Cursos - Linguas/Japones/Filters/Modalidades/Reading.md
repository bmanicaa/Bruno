...
5. **Gerar HTML, Salvar e Upload:** Formatando conforme `Filters/HTML/HTML_reading.md`, salvando em `Practice/{NIVEL}_P{X}_Reading.html` e executando o upload para o Google Drive via `upload_to_gdrive.js`. A discussão ocorrerá interativamente via chat. **Não** gere o EPUB aqui — ele é sob demanda (`Filters/Modalidades/EPUB.md`).
