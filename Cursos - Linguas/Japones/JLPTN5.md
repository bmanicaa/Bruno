...
10. **Session commands:** 
    ...
    - `"EPUB Aula N"` / `"Kindle Aula N"` → converte um artefato **já existente** na versão e-reader (EPUB 3 com furigana, para o Kindle), seguindo `Filters/Modalidades/EPUB.md`: `node scripts/build_epub.js <arquivo.html|.md> --upload`. Aceita qualquer artefato do curso — aula, Reading (HTML), Teste, Lacunas e Ditado (Markdown) — e o alvo pode ser qualificado: `"EPUB Reading Aula N"`, `"EPUB Teste Aula N"`, `"EPUB Lacunas Aula N"`, `"EPUB Ditado Aula N"`. **Só a aula é automática** (Regra 13(b2)); todo o resto é sob demanda.
