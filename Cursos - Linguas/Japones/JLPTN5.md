...
5. **`scripts/`** holds the permanent tooling:
   ...
   - `build_epub.js` — converte **qualquer artefato de estudo** (aula e Reading em HTML; Teste, Lacunas e Ditado em Markdown) num **EPUB 3 para e-reader** (Kindle Paperwhite), preservando o furigana. Roda o `validate_artifact.js` antes de escrever o arquivo: nunca entrega um livro com furigana degradado. Roda junto com o artefato pedido — a aula pela Regra 13(b2), os exercícios pela Regra 14.1 — ver `Filters/Modalidades/EPUB.md`.
...
10. **Session commands:** 
    ...
    - `"EPUB Aula N"` / `"Kindle Aula N"` → **regera** a versão e-reader de um artefato já existente (EPUB 3 com furigana, para o Kindle), seguindo `Filters/Modalidades/EPUB.md`: `node scripts/build_epub.js <arquivo.html|.md> -o "<temp>.epub" --upload`. Aceita qualquer artefato do curso — aula, Reading (HTML), Teste, Lacunas e Ditado (Markdown) — e o alvo pode ser qualificado: `"EPUB Reading Aula N"`, `"EPUB Teste Aula N"`, `"EPUB Lacunas Aula N"`, `"EPUB Ditado Aula N"`. **Este comando não é o caminho normal:** o EPUB já sai junto com o artefato pedido (Regras 13(b2) e 14.1). Ele existe para refazer a versão Kindle quando a fonte mudou — tipicamente depois de uma correção, para o gabarito chegar ao aparelho.
...
13. **MACRO "GERAR AULA" (HTML, Anki & Upload):** Ao receber a instrução "Lesson X" / "Aula X", **NÃO** imprima a aula no chat. A IA DEVE OBRIGATORIAMENTE executar o seguinte pipeline eficiente e silencioso:
    ...
    - (b2) **EPUB (versão Kindle):** Do MESMO arquivo HTML temporário, gerar e enviar a versão e-reader: `node scripts/build_epub.js "<temp_html_path>" -o "<temp_epub_path>" --upload --nome-drive "N5_LX.epub"`. O EPUB é **derivado**, nunca escrito à mão: ele reusa o mesmo `validate_artifact.js` e falha antes de escrever o arquivo se o furigana não sobreviver à conversão. Ver `Filters/HTML/HTML_Lesson.md` §4.7 para o que a conversão adapta e por quê.
    ...
    - (e) **Validação:** Rodar `node scripts/validate_artifact.js` no HTML e nos dois TSVs, corrigindo todo erro bloqueante antes de concluir. O EPUB não entra nesta lista porque `build_epub.js` já roda o mesmo validador internamente — se ele escreveu o arquivo, o artefato passou.
    - (f) **Progress.md:** Marcar a Aula X como gerada no Mapa de Progresso.
    - (g) **Conclusão:** Apagar os arquivos temporários (HTML e EPUB) e responder no chat com UMA mensagem curta confirmando os IDs do GDrive (HTML e EPUB) e a criação dos dois TSVs. A tarefa só está concluída após entregar os QUATRO artefatos.
14. **Geração e Correção Interativa de Exercícios (`Practice/`):** O sistema possui múltiplas modalidades de exercícios, todas roteadas via `Filters/Exercises.md`. Ao receber qualquer comando de exercício (`"Exercícios Aula X"`, `"Drill Aula X"`, `"Lacunas Aula X"`, etc.), a IA deve consultar `Filters/Exercises.md` para identificar a modalidade correta e seguir a especificação técnica correspondente em `Filters/Modalidades/`. Os arquivos gerados são salvos em `Practice/` com campos de resposta em branco digitáveis (`> `).
    14.1 **DOIS ARTEFATOS POR EXERCÍCIO (não é opcional):** todo comando de exercício entrega **o arquivo de trabalho E a versão Kindle**, na mesma resposta:
      - (a) o arquivo em `Practice/` — é nele, **no computador**, que o aluno digita as respostas, e é ele que os comandos `"Corrigir ..."` leem;
      - (b) a versão e-reader, gerada do MESMO arquivo: `node scripts/build_epub.js "<arquivo>" -o "<temp_epub>" --upload --nome-drive "<nome>.epub"`. Gerar em caminho temporário, subir ao Drive e apagar o local — o EPUB é derivado e o Drive é o canal de entrega (Regra 1).
      - O Kindle é **só de leitura**: nele não se digita nada. Os limites do aparelho estão em `Filters/Modalidades/EPUB.md`.
    14.2 **Correção:** Ao receber o comando de correção correspondente (`"Corrigir Aula X"`, `"Corrigir Lacunas Aula X"`, etc.), a IA lê o arquivo local, analisa as respostas digitadas pelo aluno após o caractere `>`, fornece nota e feedback didático detalhado no chat e atualiza o status do arquivo. **Depois de atualizar o arquivo, regere o EPUB** (mesmo comando de 14.1(b)): sem isso a cópia no Kindle fica sem o gabarito e ainda dizendo `⏳ Pendente` enquanto o `.md` já diz `✅`.
