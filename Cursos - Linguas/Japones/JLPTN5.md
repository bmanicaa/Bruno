...
5. **`scripts/`** holds the permanent tooling:
   ...
   - `build_epub.js` — converte a aula/Reading HTML num **EPUB 3 para e-reader** (Kindle Paperwhite), preservando o furigana. Roda o `validate_artifact.js` antes de escrever o arquivo: nunca entrega um livro com furigana degradado.
...
10. **Session commands:** 
    ...
    - `"EPUB Aula N"` / `"Kindle Aula N"` → regera **apenas** a versão e-reader de um artefato HTML já existente: `node scripts/build_epub.js <arquivo.html> --upload`. Comando de conveniência — na geração normal da aula o EPUB já sai junto pela Regra 13, sem precisar ser pedido.
