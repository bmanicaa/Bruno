...
13. **MACRO "GERAR AULA" (HTML, Anki & Upload):** Ao receber a instrução "Lesson X" / "Aula X", **NÃO** imprima a aula no chat. A IA DEVE OBRIGATORIAMENTE executar o seguinte pipeline eficiente e silencioso:
    ...
    - (b) **Upload HTML:** Salvar em um arquivo HTML temporário local e fazer upload executando o script: `node "c:\Users\bmani\Documents\GitHub\Bruno\Google Workspace\Drive\scripts\upload_to_gdrive.js" "<temp_html_path>" "N5_LX.html"` (o script valida as regras de ruby. Corrija o HTML se reprovar).
    - (c) **Anki TSV:** Gerar o arquivo complementar `.tsv` com os cards do Anki desta aula e salvar localmente (ex: `Anki/N5_L2_Anki.tsv`). O arquivo DEVE incluir obrigatoriamente no topo os cabeçalhos de automação do Anki: `#separator:tab`, `#html:true` e `#notetype:Básico`.
    - (d) **Conclusão:** Apagar o arquivo HTML temporário e responder no chat com UMA mensagem curta confirmando o ID do GDrive para o HTML e a criação do TSV. A tarefa só está concluída após entregar os DOIS artefatos.
