...
- **Dois Artefatos por Exercício (`scripts/build_epub.js`):** Toda modalidade entrega, na mesma resposta, **o arquivo de trabalho** (`.md` / `.html` em `Practice/`, onde o aluno digita as respostas no computador) **e a versão Kindle** (`.epub`, gerada do mesmo arquivo e enviada ao Drive). Ver Regra 14.1 de `JLPTN5.md` e [EPUB.md](./Modalidades/EPUB.md). Nada é gerado sem pedido: o EPUB acompanha exatamente o artefato que foi solicitado.
...
| **📱 EPUB / Kindle** *(transversal)* | **Não gera conteúdo — converte formato.** Transforma qualquer artefato acima num EPUB 3 para o Kindle Paperwhite, com o furigana preservado. Roda **junto com o artefato pedido** — a aula pela Regra 13(b2), os exercícios pela Regra 14.1. O comando avulso serve para **regerar** (ex.: depois de uma correção, para levar o gabarito ao Kindle). | [EPUB.md](./Modalidades/EPUB.md) | `"EPUB Aula X"` / `"Kindle Aula X"` · qualificado por modalidade, com `EPUB` ou `Kindle` como prefixo: `"EPUB Reading Aula X"` / `"Kindle Reading Aula X"`, `"EPUB Teste Aula X"` / `"Kindle Teste Aula X"`, `"EPUB Lacunas Aula X"` / `"Kindle Lacunas Aula X"`, `"EPUB Ditado Aula X"` / `"Kindle Ditado Aula X"` |
...
9. Para a modalidade **EPUB / Kindle**: `node scripts/build_epub.js <arquivo> --upload`. Ela é transversal e **acompanha** o artefato pedido — a aula pela Regra 13(b2), os exercícios pela Regra 14.1 —, então nos passos 5 a 8 acima o `.epub` sai junto, na mesma resposta. Invocada sozinha (`"EPUB Lacunas Aula X"`), **regera** a versão Kindle de um artefato já existente: é o que se usa depois de uma correção, para o gabarito chegar ao aparelho. Não lê nem escreve `Progress.md`: não avalia nada. Se a fonte não estiver mais em disco, baixe-a do Drive antes de converter.
10. **Antes de entregar**, a IA executa `node scripts/validate_artifact.js <arquivo gerado>` e corrige todo erro bloqueante.
