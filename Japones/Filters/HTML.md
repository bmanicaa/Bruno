...
<body>
  ...
  <header class="header-card">
    ...
    <div class="header-objective" style="border-top: none; padding-top: 0.25rem; font-size: 0.85rem;">
      <strong>✍️ Kanji da Aula (prioridade de leitura — Anki):</strong> [Kanji 1], [Kanji 2], [Kanji 3]. *(Todos os kanjis desta aula trazem furigana sempre; o recall ativo de leitura é feito no Anki.)*
    </div>
  </header>
  ...
  <!-- SEÇÃO 1: KANJI DA AULA (LEITURA) -->
  <section id="sec-1">
    <h2 class="section-title">1. 🔤 KANJI DA AULA (LEITURA)</h2>
...
3. **Escopo Total + Exceções:** A regra aplica-se a **TODO texto japonês legível** do arquivo — tabelas e collocations, exemplos de 4 camadas, diálogos, prompts e gabaritos de exercícios, exemplos ❌ errados e nomes próprios (ex.: `山田`). Exceções **isentas de ruby**:
   - Layer-4 breakdown (decomposição `[学生]`) — a frase anotada está logo acima.
   - Cards de kanji — exemplo **kunyomi** com a leitura em parênteses ao lado (`一つ` ➔ `ひと(つ)`).
   - Listas meta (contrato "✍️ Kanji da Aula" do cabeçalho).
   - Exemplos **onyomi** de cards (`一月`) NÃO são exceção — mantêm ruby.
