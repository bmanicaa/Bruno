...
13. **MACRO "GERAR AULA" (HTML, Anki & Upload):** Ao receber a instrução "Lesson X" / "Aula X", **NÃO** imprima a aula no chat. A IA DEVE OBRIGATORIAMENTE executar o seguinte pipeline eficiente e silencioso:
    ...
    - (c) **Anki TSV:** Gerar o arquivo complementar `.tsv` com os cards do Anki desta aula e salvar localmente (ex: `Anki/N5_L2_Anki.tsv`). O arquivo DEVE incluir obrigatoriamente no topo os cabeçalhos de automação do Anki: `#separator:tab`, `#html:true` e `#notetype:Basic`.
