...
7. **Lesson teaching format (content lessons):**
   ...
   - **Kanji:** Present the 3-4 priority kanji with onyomi/kunyomi and 2-3 compounds using cumulative vocabulary, **always with furigana** (rule 11). Emphasis on reading recognition; radicals/stroke count are optional reference only.
...
10. **Session commands:** 
    ...
    - `"Exercícios Aula N"` / `"Drill Aula N"` → gera o caderno de exercícios interativo em Markdown em `Practice/N5_PX.md` seguindo `Filters/Exercises.md`.
    - `"Corrigir Aula N"` / `"Avalie o Practice/N5_PX.md"` → lê o arquivo `Practice/N5_PX.md`, corrige as respostas digitadas pelo estudante, atribui nota e dá feedback detalhado no chat.
11. **POLÍTICA DE FURIGANA/RUBY (SEMPRE FURIGANA — leitura-primeiro):** Todo texto japonês gerado nas aulas segue esta regra determinística: **TODO kanji, em TODA ocorrência, recebe `<ruby>` com a leitura completa da palavra** (ruby sobre a palavra inteira, nunca kanji a kanji). O kanji é aprendido como efeito colateral da leitura por repetição; o recall ativo de leitura acontece SOMENTE no Anki (regra 7). A lista de 80 kanji de `Content/N5_Kanji.md` define apenas a **prioridade de foco nos cards do Anki** (a coluna `Aula (intro)` indica quando cada kanji entra na rotação de cards). Palavras 100% kana (sem kanji) ficam SEM ruby. Para as sub-regras técnicas completas (kana puro, escopo, exceções, omissão de layer-2-kana), seguir `Filters/HTML.md` §4.2 e §4.3.
12. **CONTRATO DE EXPECTATIVA DA AULA (promessa honesta):** Cada aula de conteúdo declara no cabeçalho os kanji prioritários da semana (3-4, conforme `Aula (intro)` em `Content/N5_Kanji.md`). **Todos** os kanji de todas as aulas são de responsabilidade de LEITURA (reconhecimento) e **sempre carregam furigana** (regra 11). **Nunca** exigir escrita produzida de kanji (de memória) — o exame N5 não testa produção de escrita; a ênfase é leitura por repetição + recall no Anki.
