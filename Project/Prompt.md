...
> - **Teste de sinal nulo (Fase E):** entradas **aleatórias** neste mesmo motor perdem R$39.598 (mediana de 300 rodadas, 4 blocos OOS); a baseline perde R$13.283 — **percentil 94 de Sharpe** contra o acaso. O sinal **tem** habilidade real. Mas o resultado dele **antes de qualquer custo é +R$311 em 3,4 anos e 243 operações**: zero. Não é caso de caçar sinal melhor **nem de cortar custo** — não há o que colher nesta família.
> - **O Projeto A (motor de swing) está CONGELADO** desde 24/08/2026 — não por refutação, mas porque nenhuma decisão pode ser tomada com este tamanho de amostra. Não modificar `backtest_institucional.py` nem os demais scripts congelados; a regra de congelamento e a lista completa estão no **`README.md`**. O trabalho pendente do projeto está no **`Plan.md`**.
