...
def run_single_asset_backtest_5pct(symbol, btc_4h, btc_1d, fng_df, start_date_str, end_date_str, risk_pct=0.015, fee_pct=0.00075):
    df_4h = compute_indicators_4h(fetch_klines_extended(symbol, interval='4h', total_candles=16000))
    df_1d = compute_indicators_1d(fetch_klines_extended(symbol, interval='1d', total_candles=3000))
    ...
    df_subset_start = df_4h[df_4h['open_time'] >= start_date]
    df_subset_end = df_4h[df_4h['open_time'] <= end_date]
    if df_subset_start.empty or df_subset_end.empty:
        return {
            'symbol': symbol.replace('USDT', ''),
            'initial_capital': initial_capital, 'final_capital': initial_capital,
            'net_profit_brl': 0.0, 'return_pct': 0.0, 'bnh_return_pct': 0.0,
    ...
    p_start = df_subset_start.iloc[0]['open']
    p_end = df_subset_end.iloc[-1]['close']
    ...
    for current_time in all_timestamps:
        ...
        if active_pos is not None:
            ...
            if not current_candle.empty:
                ...
                c_ema20_1d = 0.0
                asset_sub_1d = df_1d[df_1d['open_time'] < current_time]
                if not asset_sub_1d.empty:
                    c_ema20_1d = asset_sub_1d.iloc[-1]['ema20_1d']
                ...
                if c_low <= active_pos['stop_loss']:
                    ...
                    if not active_pos['partial_taken']:
                        ...
                        if c_high >= target_2:
                            ...
                            active_pos['exit_dates'].append(current_time)
                            active_pos['exit_prices'].append(c_close)
                            active_pos['exit_reasons'].append("Trailing EMA20 1D")
                            active_pos['final_capital'] = capital
                            active_pos['status'] = 'CLOSED'
                            trades.append(active_pos)
                            active_pos = None
...
def main():
    ...
    btc_4h = compute_indicators_4h(fetch_klines_extended('BTCUSDT', interval='4h', total_candles=16000))
    btc_1d = compute_indicators_1d(fetch_klines_extended('BTCUSDT', interval='1d', total_candles=3000))
    fng_df = fetch_fear_and_greed()
    
    scenarios = [
        ("Preliminar Rapido", "2023-10-01 00:00:00", "2024-10-01 23:59:59"),
        ("Bear Market", "2022-01-01 00:00:00", "2022-12-31 23:59:59"),
        ("Bull Market", "2023-10-01 00:00:00", "2024-03-31 23:59:59"),
        ("Lateral / Chop", "2024-04-01 00:00:00", "2024-09-30 23:59:59")
    ]
    
    for name, start, end in scenarios:
        print("="*80)
        print(f"CENÁRIO: {name} ({start} a {end})")
        print("="*80)
        results = []
        for s in symbols:
            res = run_single_asset_backtest_5pct(s, btc_4h, btc_1d, fng_df, start, end, risk_pct=0.015, fee_pct=0.00075)
            results.append(res)
            
        df_res = pd.DataFrame(results)
        total_pnl = df_res['net_profit_brl'].sum()
        avg_winrate = df_res['win_rate_pct'].mean()
        avg_drawdown = df_res['max_drawdown_pct'].mean()
        print(f"Resumo {name}: Lucro/Prejuízo: {total_pnl:.2f} | Win Rate Médio: {avg_winrate:.2f}% | Drawdown Médio: {avg_drawdown:.2f}%")
        print(df_res[['symbol', 'net_profit_brl', 'win_rate_pct', 'max_drawdown_pct']].to_string(index=False))
        print("\n")
